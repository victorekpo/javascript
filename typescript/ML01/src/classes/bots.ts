       ...
       // console.log("VALUE",value, rawValue)
        ...
        if(value && limit !== 0) {
            ...
            const itemExists = this.brain[type]?.[key]?.find(({value: val}: any) => ([val].flat().join('~') == [formattedValue].flat().join('~')
                ...
                || formattedValue.split('\r\n').filter((x: any) => !x.includes('@file')).every((line:any) => {
                    line = line.replace(' @end:','')
                    console.log('LINE', line,this.brain[type][key].includes({value:line}))
                    return this.brain[type][key].includes({value:line})
                    ...
                    || this.brain[type][key].reduce((exists: any,check: any) => {
                        if(check['value'].split(/(\@file:.*\r\n|@file:.*\....|@end:)/).filter((x: any, i: number, arr: any)=> x !== '' && x !== '\r\n' && (!arr[i]?.includes('@file') || arr[i+1])).find((i: any) => i.includes(line)))
                            exists = true;
                        return !!exists
                    },false)
                })
            ...
            if(!itemExists && (formattedValue.length > 1 || (typeof(tryToParse(this.brain[type][key]?.[value])) === 'object'))) {
                const toAdd = [formattedValue].flat().join('~').split(/\r\n|~/)
                    .filter(x => { return x.includes('@file:') ? true : !this.brain[type][key]
                        .find(({value}: any) => {
